...
use wrxswoole\Core\Helpers\ArrayHelper;
use wrxswoole\Core\Http\Formatter\CurlFormatter;
use wrxswoole\Core\Http\Formatter\JsonFormatter;
use wrxswoole\Core\Http\Formatter\UrlEncodedFormatter;
...
class Request extends Message
{
    ...
    const DEBUG_HTTP_REQUEST = "debug_request";

    const DEBUG_HTTP_REQUEST_TRACE = "Debug_Request_Trace";
    ...
    /**
     * Don't return curl response
     *
     * @var string
     */
    const RETURN_CONTENT_NONE = 'none';
     ...
     * @var string
     */
    const RETURN_CONTENT_BODY = 'body';

    /**
     * Return full response model
     *
     * @var string
     */
    const RETURN_CONTENT_FULL = 'full';
     ...
     * @var string
     */
    public $format = Request::FORMAT_URL_ENCODED;

    /**
     *
     * @var boolean
     */
    public $returnContent = Request::RETURN_CONTENT_BODY;
     ...
     */
    private $_method = Request::HTTP_METHOD_GET;
     ...
     *
     * @var array
     * @example [CURLOPT_RESOLVE => ["www.example.com:80:*.*.*.*"]],
     */
    public $curlOptions = [];

    private $content = null;

    function __construct(string $fullURL)
    {
        $this->url = $this->_fullUrl = $fullURL;
    }

    static function make($url, $enableFollowRedirect = true): Request
    {
        $request = new Request($url);
        if (! $enableFollowRedirect) {
            $request->enableFollowLocation(0);
        }

        return $request;
    }

    function get($response = null)
    {
        if (is_null($response)) {
            $response = $this->send();
        }
        ...
        if (isset($response->getHeaders()["content-type"])) {
            $contentType = strtolower($response->getHeaders()["content-type"]);

            if (strpos($contentType, 'application/json') !== false) {
                $json = json_decode($this->content, true);
                if ($this->isEnableDebug() && strlen($this->content) < 1000) {
                    $this->point(Request::DEBUG_HTTP_REQUEST_TRACE, [
                        "Url" => $this->getUrl(),
                        "Convert_Json" => $json
                    ]);
                }
    ...
    function isEnableDebug()
    {
        return strtolower(CoreCoroutineThread::getInstance()->getRequestParam(Request::DEBUG_HTTP_REQUEST)) === "true";
    }
    ...
    function post()
    {
        switch ($this->format) {
            case Request::FORMAT_JSON:
                return $this->getClient()->postJson($this->getContent());
                break;
            case Request::FORMAT_XML:
                return $this->getClient()->postXml($this->getContent());
                break;
            default:
                return $this->getClient()->post($this->getContent());
                break;
        }
    }
     ...
     * @return Request
     */
    public function setReferer($referer): Request
    {
     ...
     *            use a string to represent a URL (e.g. `http://some-domain.com`, `item/list`),
    ...
    public function getClient(): HttpClient
    {
        if (is_null($this->client)) {
            $this->client = new WHttpClient($this->_fullUrl);
            $this->client->setKeepAlive(true);
            $this->client->setTimeout(10);
        }

        $this->client->addCookies($this->cookies);
    ...
    {
        return $this->setMethod(Request::HTTP_METHOD_POST);
    }

    public function enableFollowLocation(int $maxRedirect)
    {
        $this->getClient()->enableFollowLocation($maxRedirect);
        return $this;
    }
    ...
    static function getLocation(Response $response)
    {
    ...
    {
        $content = file_getcontents($fileName);
    ...
    private function getFormatter()
    {
        switch ($this->getFormat()) {
            ...
            case Request::FORMAT_URL_ENCODED:
                return UrlEncodedFormatter::getInstance();
            default:
                return CurlFormatter::getInstance();
        }
    }
    ...
    {
        return isset($this->contentMap[$alias]) ? $this->contentMap[$alias] : $alias;
    }
