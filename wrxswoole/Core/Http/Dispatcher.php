...
use wrxswoole\Core\Component\ClassLoader;
...
class Dispatcher
{
     ...
     * @return boolean
     */
    private function isRoutingIndexController(Array $list)
    {
        if (! isset($list[0])) {
            return true;
        }
    ...
    private function controllerHandler(Request $request, Response $response, string $path)
    {
        ...
        if ($isUsingIndexController && ! $this->isRoutingIndexController($list)) {
            list ($finalClass, $actionName) = $this->loadModule($list, $finalClass, $actionName);
        }
        ...
        if (! empty($finalClass)) {
            ...
            if (is_callable($this->httpExceptionHandler)) {
                call_user_func($this->httpExceptionHandler, new PageNotFoundException([
                    [
                        "route" => $list,
                        "finalClass" => $finalClass,
                        "actionName" => $actionName
                    ]
                ], "no controller class match"), $request, $response);
            } else {
    ...
    private function loadModule($list, $finalClass, $actionName)
    {
        $id = $list[0];
        unset($list[0]);
        $module = App::getInstance()->getModule($id);
        if (is_null($module)) {
            return [
                "",
                $actionName
        ...
        return $module->createController($list);
    }
}