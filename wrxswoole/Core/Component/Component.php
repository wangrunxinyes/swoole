...
class Component extends BaseObject
{
    ...
    public function __set($name, $value)
    {
        ...
        if (method_exists($this, $setter)) {
            ...
            $this->attachBehavior($name, BaseObject::createObject($value));
    ...
    public function on($name, $handler, $data = null, $append = true)
    {
        ...
        if (strpos($name, '*') !== false) {
            if ($append || empty($this->_eventWildcards[$name])) {
                $this->_eventWildcards[$name][] = [
                    $handler,
                    $data
                ];
            } else {
                array_unshift($this->_eventWildcards[$name], [
                    $handler,
                    $data
                ]);
            }
    ...
    public function trigger($name, Event $event = null)
    {
        ...
        $eventHandlers = [];
        foreach ($this->_eventWildcards as $wildcard => $handlers) {
            if (\wrxswoole\Core\Helpers\StringHelper::matchWildcard($wildcard, $name)) {
     ...
     *            the name of the behavior. If this is an integer, it means the behavior
