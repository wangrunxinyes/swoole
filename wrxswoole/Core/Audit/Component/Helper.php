 ...
 */
namespace wrxswoole\Core\Audit\Component;

use App\App;
use wrxswoole\Core\Component\BaseVarDumper;
...
class Helper
{
    ...
    {
        if (App::getInstance()->compressData)
    ...
    public static function cleanupTrace($trace)
    {
        ...
        foreach ($trace as $n => $call) {
            ...
            $call['file'] = str_replace([
                '\\',
                '/'
            ], DIRECTORY_SEPARATOR, $call['file']);
            ...
            if (isset($call['object']))
                $call['object'] = current(self::cleanupTraceArguments([
                    $call['object']
                ]));
    ...
    public static function formatAsQuery($data)
    {
        ...
        return BaseVarDumper::dumpAsString($result, 15);
    }
