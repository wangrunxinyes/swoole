...
trait Attribute
{
    ...
    protected $withTotalCount = FALSE;
    ...
    protected $resetQuery = true;

    /** @var Table */
    protected static $schemaInfoList;
     ...
     * @return mixed
     */
    protected function reToArray($temp)
    {
    ...
    public function getAttr($attrName)
    {
        ...
        if (method_exists($this, $method)) {
            return call_user_func([
                $this,
                $method
            ], $this->data[$attrName] ?? null, $this->data);
        }
        if (isset($this->data[$attrName]))
            return $this->preCast($attrName, $this->data[$attrName]);
        if (isset($this->_joinData[$attrName]))
            return $this->preCast($attrName, $this->_joinData[$attrName]);
     ...
     * @throws Exception
     */
    protected function preCastToJson($attrName, $attrValue)
    {
     ...
     * @return mixed
     */
    protected function preCast($key, $value)
    {
        if ($this->hasCast($key)) {
            $type = $this->casts[$key];
            switch ($type) {
     ...
     * @return bool
     */
    protected function hasCast($key, $types = [])
    {
    ...
    protected function fromJson($value, $assoc = false)
    {
