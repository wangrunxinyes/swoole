 ...
 *        
 */
class PublicDbModel extends AbstractModel
{
    use TimeStamp;
    use RelationShip;
    use Attribute;
     ...
     *            $with
     * @param bool $supplyPk
     *            设置了fields 但fields中不包含需要的主键，则自动补充
     * @return $this
     */
    ...
    public function destroy($where = null, $allow = false)
    {
        ...
        $this->callEvent('onAfterDelete', $this->lastQueryResult()
            ->getAffectedRows());
    ...
    public function all($where = null)
    {
        ...
        if (DbManager::getInstance()->getConnection($this->connectionName)
            ->getConfig()
            ->isReturnCollection()) {
     ...
     * @param array $data
     * @param null $where
     * @param bool $allowUpdateWithNoCondition
     *            是否允许无条件更新
     * @return bool
     * @throws Exception
     * @throws \EasySwoole\Mysqli\Exception\Exception
    ...
    protected function lock(string $value)
    {
        $this->lock = $value;
    }
     ...
     * @throws Exception
     */
    protected function preSetWhereFromExistModel(QueryBuilder $builder)
    {
        try {
            $primaryKey = $this->schemaInfo()->getPkFiledName();
