...
class Command extends BaseObject
{
    ...
    /**
     *
     * @var \wrxswoole\Core\Database\Connection;
     */
    public $db;
    ...
    /**
     *
     * @var callable a callable (e.g. anonymous function) that is called when [[\wrxswoole\\Core\\Database\\Component\Exception]] is thrown
    ...
    {
        $this->queryCacheDuration = $duration === null ? $this->getDb()->queryCacheDuration : $duration;
     ...
     * The SQL statement will be quoted using [[Connection::quoteSql()]].
    ...
    public function setSql($sql)
    {
        if ($sql !== $this->_sql) {
            $this->cancel();
            $this->reset();
            $this->_sql = $this->getDb()->quoteSql($sql);
        }
    ...
    /**
     * Specifies the SQL statement to be executed.
     * The SQL statement will not be modified in any way.
     * The previous SQL (if any) will be discarded, and [[params]] will be cleared as well. See [[reset()]]
     * for details.
     *
     * @param string $sql
    ...
    public function getRawSql()
    {
        ...
        foreach ($this->params as $name => $value) {
            ...
            if (is_string($value)) {
                $params[$name] = $this->getDb()->quoteValue($value);
     ...
     *
     * @param bool $forRead
     *            whether this method is called for a read query. If null, it means
     ...
     *            by its PHP type. You may explicitly specify the PDO type by using a [[wrxswoole\\Core\\Database\\Component\PdoValue]] class: `new PdoValue(value, type)`,
     ...
     *
     * @param int $fetchMode
     *            the result fetch mode. Please refer to [PHP manual](https://secure.php.net/manual/en/function.PDOStatement-setFetchMode.php)
     ...
     *
     * @param int $fetchMode
     *            the result fetch mode. Please refer to [PHP manual](https://secure.php.net/manual/en/pdostatement.setfetchmode.php)
     *            for valid fetch modes. If this parameter is null, the value set in [[fetchMode]] will be used.
     * @return array|false the first row (in terms of an array) of the query result. False is returned if the query
     *         results in nothing.
     * @throws \Exception execution failed
     */
    public function queryOne($fetchMode = null): Result
    {
    ...
    {
        $table = $this->getDb()->quoteSql($table);
        $columns = array_map(function ($column) {
            return $this->getDb()->quoteSql($column);
        }, $columns);
     ...
     * @param string $type
     *            the column type. [[\wrxswoole\\Core\\Database\\Component\QueryBuilder::getColumnType()]] will be called
     ...
     * @param string|array $columns
     *            the column(s) that should be included in the index. If there are multiple columns, please separate them
    ...
    public function executeResetSequence($table, $value = null)
    {
        return $this->getDb()
            ->getQueryBuilder()
            ->executeResetSequence($table, $value);
    }
     ...
     * @param string $schema
     *            the schema name of the tables. Defaults to empty string, meaning the current
     *            or default schema.
     * @param string $table
     *            the table name.
     * @return $this the command object itself
    ...
    protected function queryInternal($method, $fetchMode = null)
    {
        if ($this->prepare()) {
            try {
                return $this->internalExecute($this->getRawSql());
            } catch (\Exception $e) {
                throw $e;
            }
        }
     ...
     * @param string $rawSql
     *            the raw SQL with parameter values inserted into the corresponding placeholders
    ...
    protected function refreshTableSchema()
    {
        if ($this->_refreshTableName !== null) {
            $this->getDb()
                ->getSchema()
                ->refreshTableSchema($this->_refreshTableName);
        }
    }
    ...
    /**
     * Sets a callable (e.g.
     * anonymous function) that is called when [[Exception]] is thrown
     * when executing the command. The signature of the callable should be:
     *
     * ```php
     * function (\wrxswoole\\Core\\Database\\Component\Exception $e, $attempt)
     * {
    ...
    {
        try {
            $result = $this->getDb()->query($this->builder, true);
            if (is_object($result->getLastError())) {
                throw $result->getLastError();
            }

            return $result;
        } catch (\Throwable $e) {
            $rawSql = $this->getRawSql() . "; By Connection: " . $this->getDb()->getConnectionName();
            $e = $this->getDb()
                ->getSchema()
