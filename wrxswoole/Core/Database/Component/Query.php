...
class Query extends BaseObject implements QueryInterface, ExpressionInterface
{
    ...
    public $connectionName = DBConfig::CONNECTION_WRITE;
    ...
    static function get($connectionName = null): Query
    {
        $query = new Query();
     ...
     *
     * @param \wrxswoole\Core\Database\DbManager $db
    ...
    {
        $connection = Connection::create($this->connectionName);
        $queryBuilder = new QueryBuilder($connection);
        list ($sql, $params) = $queryBuilder->build($this);
     ...
     * @return \wrxswoole\Core\Database\Component\Query
     */
    public function setConnectionName($connectionName): Query
    {
     ...
     */
    public function all($db = null): Result
    {
    ...
    public function one($db = null)
    {
        ...
        $result = $this->limit(1)
            ->createCommand($db)
            ->queryOne();
        return current($result->getResult());
    }
    ...
    public function exists($db = null)
    {
        ...
        $command->setSql($command->db->getQueryBuilder()
    ...
    protected function queryScalar($selectExpression, $db)
    {
        ...
        if (is_array($result) && isset($result[0][$selectExpression])) {
            return $result[0][$selectExpression];
        }
     ...
     *
     * @param string|array|ExpressionInterface $columns
     *            the columns to add to the select. See [[select()]] for more
     ...
     *            $query = (new \wrxswoole\\Core\\Database\\Component\Query)->from(['u' => 'user', 'profile']);
     *            
     *            // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     *            $subquery = (new \wrxswoole\\Core\\Database\\Component\Query)->from('user')->where(['active' => true])
     *            $query = (new \wrxswoole\\Core\\Database\\Component\Query)->from(['activeusers' => $subquery]);
     *            
     ...
     *
     * @param string|array|ExpressionInterface $condition
     *            the new WHERE condition. Please refer to [[where()]]
     ...
     *            The operator to use, when no operator is given in `$value`.
     ...
     *            the join condition that should appear in the ON part.
     ...
     *
     * @param string|array|ExpressionInterface $columns
     *            additional columns to be grouped by.
     *            Columns can be specified in either a string (e.g. "id, name") or an array (e.g. ['id', 'name']).
     *            The method will automatically quote the column names unless a column contains some parenthesis
     *            (which means the column contains a DB expression).
     *            
     ...
     *
     * @param string|array|ExpressionInterface $condition
     *            the new HAVING condition. Please refer to [[where()]]
     *            on how to specify this parameter.
     * @param array $params
     *            the parameters (name => value) to be bound to the query.
     * @return $this the query object itself
     ...
     *            the conditions that should be put in the HAVING part.
     ...
     *
     * @param array $condition
     *            the new HAVING condition. Please refer to [[having()]]
     *            on how to specify this parameter.
     * @return $this the query object itself
     * @see filterHaving()
     * @see orFilterHaving()
