 ...
 * @since 2.0
 */
class QueryBuilder extends \wrxswoole\Core\Database\Component\QueryBuilder
{
    ...
    protected function defaultExpressionBuilders()
    {
        return array_merge(parent::defaultExpressionBuilders(), [
            'wrxswoole\Core\Database\Component\JsonExpression' => 'wrxswoole\Core\Database\Component\Mysql\JsonExpressionBuilder'
        ]);
    }
     ...
     *
     * {@inheritdoc}
     * @throws \Exception this is not supported by MySQL.
     */
    public function addCheck($name, $table, $expression)
    {
        throw new \Exception(__METHOD__ . ' is not supported by MySQL.');
    }
    ...
    protected function prepareInsertValues($table, $columns, $params = [])
    {
        ...
        if (! $columns instanceof Query && empty($names)) {
            ...
            if ($tableSchema !== null) {
                $columns = ! empty($tableSchema->primaryKey) ? $tableSchema->primaryKey : [
    ...
    public function upsert($table, $insertColumns, $updateColumns, &$params)
    {
        ...
        if ($updateColumns === true) {
            ...
            $updateColumns = [
                $name => new Expression($this->db->quoteTableName($table) . '.' . $name)
            ];
        }
    ...
    public function addCommentOnColumn($table, $column, $comment)
    {
        ...
        $alterSql = 'ALTER TABLE ' . $this->db->quoteTableName($table) . ' CHANGE ' . $this->db->quoteColumnName($column) . ' ' . $this->db->quoteColumnName($column) . (empty($definition) ? '' : ' ' . $definition) . ' COMMENT ' . $this->db->quoteValue($comment);
        if ($check === 1) {
